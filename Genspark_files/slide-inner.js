(function () {
  ...
  function sendHeight() {
    try {
      ...
      // Build relative URL to match the parent page's htmlSrc prop (e.g. "./Genspark_files/filename.html")
      const filename = window.location.href.split('/').pop().split('?')[0].split('#')[0];
      const relativeUrl = './Genspark_files/' + filename;
  ...
  const SLIDE_AUDIO_EVENT_TYPE = {
    Enable: 'SLIDE_AUDIO_ENABLE',
    Disable: 'SLIDE_AUDIO_DISABLE',
    Sync: 'SLIDE_AUDIO_SYNC',
  };

  const SLIDE_AUDIO_BY_NUMBER = {
    1: '../Audios/Slide_1.m4a',
    2: '../Audios/Slide_2.m4a',
    3: '../Audios/Slide_3.m4a',
    4: '../Audios/Slide_4.m4a',
    5: '../Audios/Slide_5.m4a',
    6: '../Audios/Slide_6.m4a',
    7: '../Audios/Slide_7.m4a',
    8: '../Audios/Slide_8.m4a',
    9: '../Audios/Slide_9.m4a',
    10: '../Audios/Slide_10.m4a',
    11: '../Audios/Slide_11.m4a',
    12: '../Audios/Slide_12.m4a',
    13: '../Audios/Slide_13.m4a',
    14: '../Audios/Slide_14.m4a',
    15: '../Audios/Slide_15.m4a',
    16: '../Audios/Slide_16.m4a',
    17: '../Audios/Slide_17.m4a',
    18: '../Audios/Slide_18.m4a',
    19: '../Audios/Slide_19.m4a',
    20: '../Audios/Slide_20.m4a',
    21: '../Audios/Slide_21.m4a',
    22: '../Audios/Slide_22.m4a',
    23: '../Audios/Slide_23.m4a',
    24: '../Audios/Slide_24.m4a',
    25: '../Audios/Slide_25.m4a',
    26: '../Audios/Slide_26.m4a',
    27: '../Audios/Slide_27.m4a',
  };

  let slideAudioEnabled = false;
  let slideAudioActive = false;
  let slideAudioAutoplayPending = false;
  let visibilityCheckIntervalId = null;
  let slideNumberCache = null;
  let parentSlideLookupLogged = false;
  const slideAudioPlayer = new Audio();
  const SLIDE_AUDIO_DEBUG_PREFIX = '[slide-audio]';
  slideAudioPlayer.preload = 'auto';
  slideAudioPlayer.autoplay = false;

  function logSlideAudio(message, details) {
    const shouldLog =
      message.includes('nao foi possivel localizar o slide pai') ||
      message.includes('erro ao localizar slide pai') ||
      message.includes('nenhum') ||
      message.includes('sem arquivo') ||
      message.includes('iniciado com sucesso') ||
      message.includes('falha ao iniciar') ||
      message.includes('erro no elemento de audio');
    if (!shouldLog) {
      return
    }

    const prefix = `${SLIDE_AUDIO_DEBUG_PREFIX} [${window.location.pathname.split('/').pop()}]`;
  ...
  function getCurrentSlideRelativeUrl() {
    const filename = window.location.href.split('/').pop().split('?')[0].split('#')[0];
    return './Genspark_files/' + filename
  }

  function getSlideAudioDebugState() {
    const slideElement = getParentSlideElement();
    const rect = slideElement ? slideElement.getBoundingClientRect() : null;
    return {
      slideNumber: getSlideNumber(),
      audioSrc: getSlideAudioSrc(),
      enabled: slideAudioEnabled,
      active: slideAudioActive,
      autoplayPending: slideAudioAutoplayPending,
      readyState: slideAudioPlayer.readyState,
      networkState: slideAudioPlayer.networkState,
      paused: slideAudioPlayer.paused,
      currentTime: slideAudioPlayer.currentTime,
      ended: slideAudioPlayer.ended,
  ...
  function getParentSlideElement() {
    try {
      if (window.frameElement && window.frameElement.closest) {
        const slideElementFromFrame = window.frameElement.closest('.slide-content');
        if (slideElementFromFrame) {
          return slideElementFromFrame
        }
      }

      const parentViewer = window.parent.document.querySelector(
        `.viewer[data-src="${getCurrentSlideRelativeUrl()}"]`
      );
      const slideElement = parentViewer ? parentViewer.closest('.slide-content') : null;
      if (!slideElement && !parentSlideLookupLogged) {
        parentSlideLookupLogged = true;
        logSlideAudio('nao foi possivel localizar o slide pai', {
          relativeUrl: getCurrentSlideRelativeUrl(),
        });
      }
      return slideElement
    } catch (error) {
      if (!parentSlideLookupLogged) {
        parentSlideLookupLogged = true;
        logSlideAudio('erro ao localizar slide pai', {
          relativeUrl: getCurrentSlideRelativeUrl(),
  ...
  function getSlideNumber() {
    if (slideNumberCache !== null) {
      return slideNumberCache
    }
    ...
    if (header) {
      ...
      if (match) {
        slideNumberCache = parseInt(match[1], 10);
        return slideNumberCache
      }
    }

    try {
      const slideElements = Array.from(window.parent.document.querySelectorAll('.slide-content'));
      const index = slideElements.indexOf(slideElement);
      if (index >= 0) {
        slideNumberCache = index + 1;
        return slideNumberCache
      }
    } catch (error) {}
  ...
  function isSlideVisibleInPresentation() {
    if (document.hidden || !slideAudioEnabled) {
      return false
    }
    ...
    const viewportWidth = window.parent.innerWidth || document.documentElement.clientWidth || 0;
    const viewportHeight = window.parent.innerHeight || document.documentElement.clientHeight || 0;
    ...
    const slideArea = rect.width * rect.height;
    const visibleRatio = slideArea > 0 ? visibleArea / slideArea : 0;
    ...
    return visibleRatio >= 0.6 || containsViewportCenter
  }

  function stopSlideAudio() {
    slideAudioPlayer.pause();
    slideAudioPlayer.currentTime = 0;
    slideAudioActive = false;
    slideAudioAutoplayPending = false;
  }

  function playSlideAudio() {
    const audioSrc = getSlideAudioSrc();
    if (!audioSrc) {
      logSlideAudio('nenhum áudio mapeado para este slide', getSlideAudioDebugState());
      stopSlideAudio();
      return
    }

    if (slideAudioPlayer.getAttribute('src') !== audioSrc) {
      logSlideAudio('definindo src do áudio', { previousSrc: slideAudioPlayer.getAttribute('src'), nextSrc: audioSrc });
      slideAudioPlayer.src = audioSrc;
    }

    slideAudioPlayer.currentTime = 0;
    logSlideAudio('tentando reproduzir áudio', getSlideAudioDebugState());
    slideAudioPlayer.play().then(() => {
      slideAudioActive = true;
      slideAudioAutoplayPending = false;
      logSlideAudio('áudio iniciado com sucesso', getSlideAudioDebugState());
    }).catch(error => {
      slideAudioActive = false;
      slideAudioAutoplayPending = true;
      logSlideAudio('falha ao iniciar áudio', {
        ...getSlideAudioDebugState(),
        errorName: error && error.name,
        errorMessage: error && error.message,
      });
      console.warn('Failed to start slide audio:', error);
    });
  }

  function updateSlideAudio(forceRestart) {
    const visible = isSlideVisibleInPresentation();

    if (!slideAudioEnabled || !visible) {
      if (slideAudioActive || slideAudioAutoplayPending) {
        stopSlideAudio();
      }
      return
    }
    ...
    const audioSrc = getSlideAudioSrc();
    if (!audioSrc) {
      logSlideAudio('updateSlideAudio sem arquivo de áudio', getSlideAudioDebugState());
      stopSlideAudio();
      return
    }
    ...
    if (
      !slideAudioActive ||
      slideAudioAutoplayPending ||
      forceRestart ||
      slideAudioPlayer.getAttribute('src') !== audioSrc
    ) {
      playSlideAudio();
    }
  }

  function enableSlideAudio() {
    const wasEnabled = slideAudioEnabled;
    slideAudioEnabled = true;
    updateSlideAudio(!wasEnabled);
  }

  function disableSlideAudio() {
    slideAudioEnabled = false;
    stopSlideAudio();
  }

  function startSlideAudioVisibilityWatcher() {
    if (visibilityCheckIntervalId !== null) {
      return
    }

    visibilityCheckIntervalId = window.setInterval(() => {
      updateSlideAudio(false);
    }, 200);
  }

  function resumePendingSlideAutoplay() {
    if (!slideAudioEnabled || !slideAudioAutoplayPending) {
      return
    }
    updateSlideAudio(true);
  }
...
`;
  ...
  function hanlderMessage(event) {
    if (event.data && event.data.type === SLIDE_AUDIO_EVENT_TYPE.Enable) {
      enableSlideAudio();
      return
    }

    if (event.data && event.data.type === SLIDE_AUDIO_EVENT_TYPE.Disable) {
      disableSlideAudio();
      return
    }

    if (event.data && event.data.type === SLIDE_AUDIO_EVENT_TYPE.Sync) {
      updateSlideAudio(false);
      return
    }
  ...
  window.addEventListener('resize', () => updateSlideAudio(false));
  document.addEventListener('wheel', hanlderWheel);
  document.addEventListener('keydown', hanlderKeyDown);
  document.addEventListener('visibilitychange', () => updateSlideAudio(false));
  window.addEventListener('pagehide', disableSlideAudio);
  window.addEventListener('beforeunload', disableSlideAudio);
  ['pointerdown', 'click', 'touchstart', 'keydown'].forEach(eventName => {
    document.addEventListener(eventName, resumePendingSlideAutoplay, true);
  });

  sendHeight();
  slideSelectedEdit();
  startSlideAudioVisibilityWatcher();
  ...
  [
    ...
    slideAudioPlayer.addEventListener(eventName, () => {
      const mediaError = slideAudioPlayer.error;
      logSlideAudio(`evento do áudio: ${eventName}`, {
        ...getSlideAudioDebugState(),
